...
torch.save(model.state_dict(), save_train / "faster_rcnn.pth")
...
with torch.no_grad():
    for imgs, trgts in valid_loader:
        ...
        for out, tgt in zip(outputs, trgts):
            predictions.append({        # Stocker les prédictions de bounding box (format COCO), les labels (classes) et les scores de confiance
                "boxes": out["boxes"].cpu(),
                "labels": out["labels"].cpu(),
                "scores": out["scores"].cpu()
            })
            targets.append({            # stocker les cibles réelles des bounding box et les labels (classes)
                "boxes": tgt["boxes"].cpu(),
